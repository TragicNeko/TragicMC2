...
public class WorldGenBleachedTree extends WorldGenAbstractTree {
	...
	public boolean generate(World world, Random random, int x, int y, int z) {
		...
		if (!WorldHelper.validBlocksForDimension.contains(world.getBlock(x, y, z))) return false;
		...
		Map<Integer, int[]> map = WorldHelper.getBlocksInSphericalRange(world, size * 0.685D, x, y + (size * 0.785D), z);
		int[] coords;
		Block block;

		for (int i = 0; i < map.size(); i++) //creates a dome shaped leaf coverage with less chance for leaves at lower heights to be placed
		{
			coords = map.get(i);
		...
		map = WorldHelper.getBlocksInSphericalRange(world, size * 0.515D, x, y + (size * 0.785D) - 1.5D, z);

		for (int i = 0; i < map.size(); i++) //removes extra leaves that are too low
		{
			coords = map.get(i);
			block = world.getBlock(coords[0], coords[1], coords[2]);
		...
		{
			map = WorldHelper.getBlocksInCircularRange(world, size * 0.267D, x, y + y1, z);
			...
			{
				map = WorldHelper.getBlocksInCircularRange(world, size * 0.487D, x, y + y1, z);
			...
			if (y1 <= size * 0.145D)
			{
				map = WorldHelper.getBlocksInCircularRange(world, size * 0.638D, x, y + y1, z);
			...
			if (y1 >= size * 0.925D)
			{
				map = WorldHelper.getBlocksInCircularRange(world, size * 0.618D, x, y + y1, z);
			...
			if (y1 >= size * 0.887D) //remove some blocks from the inside near the top to give it more of an open look
			{
				...
				map = WorldHelper.getBlocksInCircularRange(world, size * 0.283D, x, y + y1, z);
			...
			{
				map = WorldHelper.getBlocksInCircularRange(world, size * 0.363D, x, y + y1, z);
		...
		map = WorldHelper.getBlocksInCircularRange(world, size * 0.728D, x, y - 1, z); //to make the roots appear more widespread
		...
		{
			map = WorldHelper.getBlocksInCircularRange(world, size * 0.838D, x, y - 2, z);
